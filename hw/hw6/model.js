...
exports.getCategories = (callback) => {
    db.all("SELECT name FROM Categories", [], (err, rows) => callback(err, rows));
};
...
exports.getJokesByCategory = (category, limit, callback) => {
    const query = `
        ...
        LIMIT ?
    `;
    db.all(query, [category, limit], (err, rows) => callback(err, rows));
};
...
exports.addJoke = (category, setup, delivery, callback) => {
    ...
    `;
    db.run(query, [setup, delivery, category], function (err) {
